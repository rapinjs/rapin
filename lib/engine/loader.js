...
class Loader {
    ...
    controller(route, data) {
        return __awaiter(this, void 0, void 0, function* () {
            event_1.triggerEvent('controller/' + route, 'before', { data, route });
            const action = new action_1.default(route);
            const output = yield action.execute(this.registry, data);
            event_1.triggerEvent('controller/' + route, 'after', { data, route, output });
            return output;
        });
    }
    model(route) {
    ...
    view(route, data) {
        ...
        event_1.triggerEvent('view/' + route, 'before', { route, data });
        const output = template.render(route);
        event_1.triggerEvent('view/' + route, 'after', { route, data, output });
        return output;
    }
    config(route) {
